...
class ModifyExamPage extends StatefulWidget {
  ...
  const ModifyExamPage(
      {super.key, required this.examId, required this.groupId});
  ...
  void _submitTest(BuildContext context) {
    ...
    if (title.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("測驗標題不能為空")),
      );
      return;
    }
    ...
    if (timeLimit.isEmpty || int.tryParse(timeLimit) == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("時間限制必須為有效數字")),
      );
      return;
    }
      ...
      body: BlocListener<ExamBloc, ExamState>(
        listener: (context, state) {
          if (state is LoadExamByIdSuccess) {
            ...
            ScaffoldMessenger.of(context)
                .showSnackBar(const SnackBar(content: Text('測驗修改成功')));
          ...
          child: Column(
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(labelText: '測驗標題'),
              ),
              TextField(
                controller: _timeLimitController,
                keyboardType: TextInputType.number,
                decoration: const InputDecoration(labelText: '時間限制(分鐘)'),
              ),
